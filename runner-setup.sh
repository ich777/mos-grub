...
GH_RUNNER_V=2.328.0
mkdir -p /root/runner/mos-grub /root/runner_workdir/mos-grub
cd /root/runner/mos-grub
...
RUNNER_ALLOW_RUNASROOT="1" ./config.sh --url https://github.com/ich777/mos-grub --name mos-grub --work /root/runner_workdir/mos-grub --unattended --token <YOURTOKEN>

# Copy over runner-mos_grub to /etc/init.d/ and make sure it's executable
echo "/etc/init.d/runner-mos_grub start"  >> /etc/rc.local
